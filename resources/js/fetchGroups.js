...
let debounceTimeout;
let currentQuery = urlParams.get('search') || '';
let currentCategory = '';
...
async function clearGroupCardsWithAnimation() {
    const cards = document.querySelectorAll('#groups-grid > div');
    const promises = [];
...
async function fetchGroups(page = 1, query = currentQuery, selectedCategory = currentCategory, teacher = currentTeacher, levels = currentLevels, classes = currentClass, address = currentAddress, date = currentDate, time = currentTime, sort = currentSort, field = sortField) {
    ...
    const groupList = document.getElementById('groups-grid');
    const scrollLoader = document.getElementById('scrollLoader');

    try {
        const request = await fetch(`/groups/index?page=${page}
        ...
        response.data.forEach(group => {
            const card = `
                    ...
                    <div class="flex items-center justify-between p-4">
                        <a href="/teacher/${group.user.id}">
                            <div class="flex items-center space-x-3">
                                <img
                                    src="${group.user.photo_profile}"
                                    alt="${group.user.name}" class="w-10 h-10 rounded-full object-cover">
                    ...
                    <div class="video-wrapper relative aspect-9/16">
                        <video playsinline loop
                               class="custom-video w-full h-full object-cover"
                                ...
                                <source src="${group.video_group}" type="video/mp4"></video>
                    ...
                    <div class="flex justify-between items-center p-4">
                        <div class="flex space-x-4">
                            ...
                            <div class="tooltip size-8 sm:size-7" data-tip="${group.isFavorite ? 'Убрать из избранного' : 'Избранное'}">
                                <button class="favorite-btn hover:text-red-500 ${group.isFavorite ? 'text-red-500' : ''}"
                                data-id="${group.id}" data-favorite="${group.isFavorite}">
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="${group.isFavorite ? 'currentColor' : 'none'}" viewBox="0 0 24 24"
                            ...
                            <div class="tooltip size-8 sm:size-7" data-tip="Чат">
                                ${group.isAuth ? `<a href="/chat?user=${group.user.id}" class="hover:text-gray-500">
                                ...
                                </a>` : `<button onclick="authModal.showModal()">
                                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-8 sm:size-7">
                                        <path stroke-linecap="round" stroke-linejoin="round" d="M21.75 6.75v10.5a2.25 2.25 0 0 1-2.25 2.25h-15a2.25 2.25 0 0 1-2.25-2.25V6.75m19.5 0A2.25 2.25 0 0 0 19.5 4.5h-15a2.25 2.25 0 0 0-2.25 2.25m19.5 0v.243a2.25 2.25 0 0 1-1.07 1.916l-7.5 4.615a2.25 2.25 0 0 1-2.36 0L3.32 8.91a2.25 2.25 0 0 1-1.07-1.916V6.75" />
                                    </svg>
                                         </button>`}
                    ...
                    <div class="desc-post pb-4 px-4">
                        ...
                        <button class="toggle-btn text-blue-500 text-sm mt-1 hover:underline">Показать полностью</button>
                    </div>
...
if (window.location.pathname === '/groups') {
    ...
    document.querySelectorAll('input[name="teachers[]"]').forEach(checkbox => {
        checkbox.addEventListener('change', async () => {
            const selectedTeacher = Array.from(document.querySelectorAll('input[name="teachers[]"]:checked'))
                .map(cb => cb.value)
                .join(',');
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        const clearBtnTime = document.getElementById('clearTime');
        ...
        function toggleTimeIcons() {
            if (select.value.trim()) {
                clearBtnTime.classList.remove('hidden');
                timeIcon.classList.add('hidden');
            } else {
                clearBtnTime.classList.add('hidden');
                timeIcon.classList.remove('hidden');
            }
        }

        select.addEventListener('change', toggleTimeIcons);

        clearBtnTime.addEventListener('click', async () => {
            select.value = '';
            toggleTimeIcons();
            currentTime = '';
        ...
        toggleTimeIcons(); // начальное состояние
    });
    ...
    const picker = new Pikaday({
        field: dateInput, format: 'dd, DD MMMM YYYY', bound: true, firstDay: 1, i18n: {
            ...
            const weekdayShort = this._o.i18n.weekdaysShort[date.getDay()];
            ...
            currentDate = `${weekdayShort}, ${localDate}`;
            toggleDateIcons();
            showScrollLoader();
            currentPage = 1;
            lastPage = false;
    ...
    document.getElementById('time').addEventListener('input', async function (event) {
